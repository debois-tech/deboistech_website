...
function findBlogPostById(id, options = {}) {
  ...
  if (id) return posts.find((post) => post.id === id || post.slug === id) || null;
...
function renderBlogDetail(post, basePath, statsOverride) {
  const p = basePath || ".";
  if (!post) return `<div class="flex items-center justify-center py-32 text-gray-500 text-lg">Blog post not found.</div>`;
    ...
    <article class="bg-white px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
      <div class="mx-auto max-w-4xl">
        ...
        <header class="mt-10">
          ...
          ${isAdmin ? `<div class="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
    ...
    <div class="flex flex-col min-h-screen bg-white font-sans text-gray-900">
      <header class="flex items-center justify-between px-6 py-3 border-b border-gray-100 sticky top-0 bg-white z-50">
        <div class="flex items-center gap-4">
          <a href="${blogPageHref}" class="flex items-center"><img src="${basePath || "."}/images/logo.png" alt="deboistech" class="h-7 w-auto" /></a>
...
function initBlogStudio({ pageHref }) {
  ...
  form.addEventListener("submit", (event) => {
    ...
    const slug = fields.id.value;
    const detailPage = pageHref ? `${pageHref}/blog-detail.html` : "blog-detail.html";
    window.location.href = `${detailPage}?post=${encodeURIComponent(slug)}`;
  });
  ...
  // Hidden file input for image upload (no server — renders inline via data URL)
  ...
  imageFileInput.addEventListener("change", () => {
    ...
    reader.onload = (ev) => {
      ...
      editor.focus();
      document.execCommand("insertHTML", false, `<figure contenteditable="false"><img src="${dataUrl}" alt="${escapeHtml(file.name)}" style="max-width:100%;border-radius:12px;" /></figure><p><br></p>`);
      toolbar.classList.add("hidden");
      plusBtn.classList.add("hidden");
    };
