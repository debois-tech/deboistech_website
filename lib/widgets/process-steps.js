...
function renderProcessSteps({ mountId, steps }) {
  ...
  const stepItems = steps
    .map(
      (step) => `
    <div class="group relative z-10 flex flex-col items-center text-center w-full flex-shrink-0 snap-center snap-always md:w-48 md:snap-align-none">
  ...
  el.innerHTML = `
    <section class="reveal bg-white px-4 py-20 sm:px-6 sm:py-28 lg:px-8">
      <div class="mx-auto max-w-7xl">
        ...
        <div class="mt-16">
          <div class="process-steps-scroll relative flex items-start justify-between overflow-x-auto snap-x snap-mandatory md:snap-none">
            <div class="absolute left-0 top-9 h-0.5 w-full border-t-2 border-dashed border-primary-200" data-connector-line></div>
            ${stepItems}
          </div>
        </div>
      </div>
  ...
  requestAnimationFrame(() => {
    const container = el.querySelector(".process-steps-scroll");
    const line = container?.querySelector("[data-connector-line]");
    if (!container || !line) return;

    const syncLine = () => {
      line.style.width = `${Math.max(container.scrollWidth, container.clientWidth)}px`;
    };
    syncLine();
    new ResizeObserver(syncLine).observe(container);
  });
}
