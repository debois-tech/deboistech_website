...
function renderBlogSections({ mountId, sections, bg, basePath, detailHref, editorHref }) {
  ...
  const sectionsHTML = sectionData
    .map((section) => {
      const cardsHTML = section.blogs
        .map((blog) => {
          const card = typeof renderBlogCard === "function" ? renderBlogCard(blog, basePath, detailHref) : "";
          if (!isAdmin || !blog.id) return card;
          const deleteBtn = `
            <div class="mt-2 flex justify-end">
              <button type="button" data-delete-blog="${encodeURIComponent(blog.id)}"
                class="text-xs text-red-500 hover:text-red-700 hover:bg-red-50 rounded-full px-3 py-1 transition font-medium">
                Delete
              </button>
            </div>`;
          return `<div class="flex flex-col">${card}${deleteBtn}</div>`;
        })
        .join("\n");
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", async (e) => {
      ...
      if (typeof teamLogin === "function" && await teamLogin(pwd)) {
  ...
  // Delete blog from listing page
  el.addEventListener("click", (e) => {
    const deleteBtn = e.target.closest("[data-delete-blog]");
    if (!deleteBtn) return;
    e.preventDefault();
    const id = decodeURIComponent(deleteBtn.dataset.deleteBlog);
    if (confirm("Delete this story? This cannot be undone.") && typeof deleteStoredBlogPost === "function") {
      deleteStoredBlogPost(id);
      window.location.reload();
    }
  });
}
